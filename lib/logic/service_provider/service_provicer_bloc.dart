...
class ServiceProvidersBloc extends Cubit<ServiceProvidersState> {
  ...
  updateQuery(double value) async {
    ...
    emit(ServiceProvidersState.success(allMarkers: allMarkers, rad: radius.value));
    if (lastRadius != value) {
      radius.add(value);
      lastRadius = value;
    }
  ...
  void _updateMarkers(List<DocumentSnapshot> docList) async {
    allMarkers = [];
  ...
  getpros({required String serviceId}) async {
    ...
    try {
      ...
      await ServiceProvidersServ.startQuery(serviceId: serviceId, radius: radius, position: myPosition!).then((value) {
        value.listen(_updateMarkers);
      });
